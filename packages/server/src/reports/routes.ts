...
import { studentIdsForUser } from '../trpc/familyAccess';
import { renderClassCombined } from './generate';
import { reportFilePath } from './pdf';

/** May the current request fetch this artifact as a PARENT? Only their own kid's, and only when
 *  PUBLISHED (CLAUDE.md §5/§14 — minors' academic records; parents see published only). */
function parentMayFetch(req: FastifyRequest, studentId: string, publishedAt: Date | null): boolean {
  ...
  if (!roleAllowedFromOrigin('parent', classifyOrigin(req))) return false;
  if (!publishedAt) return false;
  return studentIdsForUser(session.userId).includes(studentId);
}
...
export function registerReportRoutes(app: FastifyInstance): void {
  ...
  app.get('/reports/card/:id', async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const card = db.select({ id: reportCards.id, studentId: reportCards.studentId, classId: reportCards.classId, version: reportCards.version, publishedAt: reportCards.publishedAt, pdfPath: reportCards.pdfPath }).from(reportCards).where(eq(reportCards.id, req.params.id)).get();
    if (!card) return reply.code(404).send({ error: 'Not found.' });
    if (!canAccessClass(req, card.classId) && !parentMayFetch(req, card.studentId, card.publishedAt)) return reply.code(403).send({ error: 'You don’t have access to that.' });
    const file = reportFilePath(card.pdfPath);
    if (!fs.existsSync(file)) return reply.code(404).send({ error: 'Not found.' });
    return reply.header('Content-Type', 'application/pdf').header('Cache-Control', 'no-store').header('Content-Disposition', `inline; filename="report-card-v${card.version}.pdf"`).send(fs.readFileSync(file));
  });
  ...
  app.get('/reports/transcript/:id', async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const tr = db.select({ id: transcripts.id, studentId: transcripts.studentId, version: transcripts.version, publishedAt: transcripts.publishedAt, pdfPath: transcripts.pdfPath }).from(transcripts).where(eq(transcripts.id, req.params.id)).get();
    if (!tr) return reply.code(404).send({ error: 'Not found.' });
    if (!canAccessTranscript(req) && !parentMayFetch(req, tr.studentId, tr.publishedAt)) return reply.code(403).send({ error: 'You don’t have access to that.' });
    const file = reportFilePath(tr.pdfPath);
    if (!fs.existsSync(file)) return reply.code(404).send({ error: 'Not found.' });
    return reply.header('Content-Type', 'application/pdf').header('Cache-Control', 'no-store').header('Content-Disposition', `inline; filename="transcript-v${tr.version}.pdf"`).send(fs.readFileSync(file));
  });
  ...
  app.get('/reports/class/:classId/combined', async (req: FastifyRequest<{ Params: { classId: string } }>, reply: FastifyReply) => {
    ...
    return reply.header('Content-Type', 'application/pdf').header('Cache-Control', 'no-store').header('Content-Disposition', 'inline; filename="report-cards.pdf"').send(buf);
  });
}
